...
require(charter.get("organization") == "OmniBlitz", "organization must be OmniBlitz")
require(charter.get("canonical_linear_project") == "github.com/OmniBlitz", "canonical Linear project mismatch")
...
if isinstance(repos, list):
    require(all(isinstance(repo, str) and repo.startswith("OmniBlitz/") for repo in repos), "approved repositories must stay inside OmniBlitz")
require(nonempty_strings(charter.get("prohibited_assumptions")), "prohibited_assumptions must be explicit")

if status == "unresolved":
    require(repos == ["OmniBlitz/.github"], "unresolved charter may approve only OmniBlitz/.github")
